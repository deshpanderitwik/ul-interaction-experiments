...
import { ROOT_NAMES, SCALE_LABELS, SCALE_TYPES, getScale, setScale, useScale } from '../scale';

// Global scale picker for the settings side sheet: a root (all 12 chromatic
// roots) plus mode (the seven diatonic modes — Major/Ionian … Locrian). Changing
// it re-pitches every experiment's notes.
      ...
      <View style={styles.quality}>
        {SCALE_TYPES.map((q) => {
          ...
          return (
            ...
            >
              <Text style={[styles.qualText, on && styles.selectedText]}>{SCALE_LABELS[q]}</Text>
            </Pressable>
          );
        })}
      </View>
...
const styles = StyleSheet.create({
  ...
  quality: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 },
  qual: {
    height: 40,
