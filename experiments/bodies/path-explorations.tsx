...
// Bodies · Path Explorations — a fork of Bent Paths. The path's terminals (its two
// endpoints) render as solid discs and long-press to open the subdivision / move
// radial; an interior waypoint renders as a hollow ring and long-presses to open a
// two-option radial: STATIC (default — stays put) or DYNAMIC. A dynamic point
// wanders to a neighboring scale note each time the body passes it (bounded to ±2
// steps from home), so the body explores variations around the path each cycle.
// Plucking (drag a segment's length) still works as in Bent Paths.
...
// What a radial wedge selects: a subdivision denominator, delete/extend, or the
// static/dynamic mode of an interior waypoint.
...
export default function PathExplorations() {
  ...
  const openRadialWith = (p: Placing, seed: RadialSel | null) => {
  ...
  // Subdivision / edit radial for a body (targetId null = placing a new body).
  const openSubRadial = (x: number, y: number, targetId: number | null) => {
    ...
    openRadialWith({ x, y, cx, cy, mode: 'sub', targetId, hasPath }, seed);
  };
  ...
  const onLongPress = (x: number, y: number) => {
    ...
    if (wp) {
      ...
      const terminal = !b?.path || wp.index === 0 || wp.index === b.path.length - 1;
      if (terminal) openSubRadial(x, y, wp.bodyId);
      else openDynRadial(x, y, wp.bodyId, wp.index);
      return;
    }
    const id = hitId(x, y);
    if (id != null) openSubRadial(x, y, id);
    else if (hitPathSegment(x, y)) return; // on a path's length → reserved for the pluck drag
    else if (x >= RULER_WIDTH) openSubRadial(x, y, null);
  };
    ...
    <View style={styles.fill}>
      ...
      {placing ? (
        <SubdivisionRadial
          ...
          hover={hoverSub}
          closing={closing}
          ghost={placing.mode === 'sub' && placing.targetId == null}
...
function SubdivisionRadial({
  ...
  const items = radialItems(placing);
...
function PathLine({
  ...
  return (
    <Group opacity={dim ? 0.25 : 1}>
      ...
      {handles
        ? points.map((wp, i) => {
            const cy = yForMidiExt(wp.midi, full, scroll, visible, height);
            ...
            if (wp.dyn) {
              ...
              return (
                <Group key={i}>
                  <Circle cx={wp.x} cy={cy} r={HANDLE_R} style="stroke" strokeWidth={2} color="rgba(255,255,255,0.6)" />
                  <Circle cx={wp.x} cy={cy} r={HANDLE_R * 0.42} color="rgba(255,255,255,0.85)" />
                </Group>
              );
            }
            // Static interior point: hollow ring.
            return (
              <Circle key={i} cx={wp.x} cy={cy} r={HANDLE_R} style="stroke" strokeWidth={2} color="rgba(255,255,255,0.6)" />
            );
          })
        : null}
    </Group>
