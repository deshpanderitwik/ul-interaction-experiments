...
**OTA channel + runtime** (`preview` channel @ `runtimeVersion 1.3.0`). EAS
...
- **Native** (Swift in `modules/`, a new native dependency, `ios.infoPlist`, the
  ...
  reaches installs on the **matching `runtimeVersion`** (`1.3.0`), so bumping the
...
`runtimeVersion 1.3.0`, channel `preview`. These pin the installed app and its
